...
print('№5:')  # Ответ: 9
for orig in range(1, 50):  # Рассматриваем разные нат. числа, обычно их немного
    new = bin(orig)[2:]  # Строим их двоичную запись, срезав указатель сс
    new += '0' if orig % 2 == 0 else '1'  # Шаг 2 через тернарку (do ... if True else don't ...)
    new = new.replace(new[:2], '11', 1) if new.count('1') % 3 == 0 else new.replace(new[:2], '10', 1)  # По аналогии
    if int(new, 2) >= 26:  # Ограничение из условия
        print(orig, ', а новое такое:', int(new, 2))  # Ради интереса выводим и N, и R
...
up() # Поднимаем хвост, чтобы не считать точку выхода
...
print('№9:')  # Ответ: 1
# Открываем файл. Если условие - кукож, то нажимаем F12, сохраняем в .txt с табуляцией (.csv тоже в тему)
counter = 0  # Счётчик кол-ва строк, удовлетворяющих требованию
with open('C:/for типовые 20 вариантов/9/z9_v1-4.txt', "r") as file9:  # Этот способ САМ закроет файл после проги, не нагружая память
...
for A in range(1, 1000):  # Классическое решение (через флажковый метод):
    flag = True  # или 1
    for x in range(1, 100000):  # Иксов всегда перебираем побольше параметра
...
with open('C:/for типовые 20 вариантов/17/17var01.txt', "r") as file17:
    ...
    for number, number_next in zip(file17, file17[1:]):  # Инициализируем по паре чисел из списка (схоже для троек)
        if number + number_next == maximum:  # Критерии поиска
            counter += 1
...
def heap19(rocks1, positions):
    ...
    else:  # Если есть "ПРИ ЛЮБОМ ХОДЕ ПЕРВОГО ВТОРОЙ МОЖЕТ ВЫИГРАТЬ ЗА 1 ХОД", то учитываем чужие ходы с И
        return heap19(rocks1 + 1, positions + 1) and heap19(rocks1 * 2, positions + 1)


for answer19 in range(1, 228 + 1):  # Перебираем кол-во камней в куче
...
with open('C:/for типовые 20 вариантов/24/24var01.txt', "r") as file24:
    f = file24.read() # Читаем весь файл как нравится
